import { readdir } from "node:fs/promises";
import { join } from "node:path";

const POLL_INTERVAL_MS = 5_000;
const STEAM_ROOT = join(process.env.HOME!, ".local/share/Steam");
const STEAM_USER_ID = process.env.STEAM_USER_ID;

function parseVdfPairs(content: string): Record<string, string> {
  const result: Record<string, string> = {};
  for (const match of content.matchAll(/^\s+"(\w+)"\s+"([^"]*?)"/gm)) {
    result[match[1]!.toLowerCase()] = match[2]!;
  }
  return result;
}

async function getLibraryPaths(): Promise<string[]> {
  const content = await Bun.file(join(STEAM_ROOT, "steamapps/libraryfolders.vdf")).text();
  const paths = [join(STEAM_ROOT, "steamapps")];
  for (const match of content.matchAll(/"path"\s+"([^"]+)"/g)) {
    paths.push(join(match[1]!, "steamapps"));
  }
  return [...new Set(paths)];
}

async function buildAppIdMap(libraryPaths: string[]): Promise<Map<string, string>> {
  const map = new Map<string, string>();
  for (const libPath of libraryPaths) {
    let entries: string[];
    try {
      entries = await readdir(libPath);
    } catch {
      continue;
    }
    for (const entry of entries) {
      if (!entry.startsWith("appmanifest_") || !entry.endsWith(".acf")) continue;
      try {
        const content = await Bun.file(join(libPath, entry)).text();
        const values = parseVdfPairs(content);
        if (values.appid && values.name) map.set(values.appid, values.name);
      } catch {
        // skip unreadable manifests
      }
    }
  }
  return map;
}

function parseShortcutsVdf(buf: Uint8Array): Map<string, string> {
  let pos = 0;

  function readCString(): string {
    const start = pos;
    while (pos < buf.length && buf[pos] !== 0) pos++;
    const str = new TextDecoder().decode(buf.slice(start, pos));
    pos++;
    return str;
  }

  function readU32(): number {
    const val =
      (buf[pos]! | (buf[pos + 1]! << 8) | (buf[pos + 2]! << 16) | (buf[pos + 3]! << 24)) >>> 0;
    pos += 4;
    return val;
  }

  function skipValue(type: number): void {
    if (type === 0x01) {
      readCString();
    } else if (type === 0x02) {
      pos += 4;
    } else if (type === 0x00) {
      // nested object — recurse until closing \x08
      while (pos < buf.length && buf[pos] !== 0x08) {
        const t = buf[pos++]!;
        readCString();
        skipValue(t);
      }
      pos++; // consume \x08
    }
  }

  const map = new Map<string, string>();
  if (buf[pos] !== 0x00) return map;
  pos++;
  readCString(); // "shortcuts"

  while (pos < buf.length && buf[pos] !== 0x08) {
    const type = buf[pos++]!;
    readCString(); // shortcut index "0", "1", etc.

    if (type !== 0x00) {
      skipValue(type);
      continue;
    }

    let appId: number | null = null;
    let appName: string | null = null;

    while (pos < buf.length && buf[pos] !== 0x08) {
      const t = buf[pos++]!;
      const field = readCString();
      if (t === 0x02 && field.toLowerCase() === "appid") {
        appId = readU32();
      } else if (t === 0x01 && field.toLowerCase() === "appname") {
        appName = readCString();
      } else {
        skipValue(t);
      }
    }
    pos++; // consume closing \x08

    if (appId !== null && appName) map.set(String(appId), appName);
  }

  return map;
}

async function loadShortcuts(): Promise<Map<string, string>> {
  if (!STEAM_USER_ID) return new Map();
  // Lower 32 bits of the steamid64 is the account ID used for userdata paths
  const accountId = Number(BigInt(STEAM_USER_ID) & 0xffffffffn);
  try {
    const buf = await Bun.file(
      join(STEAM_ROOT, `userdata/${accountId}/config/shortcuts.vdf`),
    ).arrayBuffer();
    return parseShortcutsVdf(new Uint8Array(buf));
  } catch {
    return new Map();
  }
}

async function getRunningGame(
  appIdMap: Map<string, string>,
  shortcuts: Map<string, string>,
): Promise<string | null> {
  let pids: string[];
  try {
    pids = await readdir("/proc");
  } catch {
    return null;
  }

  for (const pid of pids) {
    if (!/^\d+$/.test(pid)) continue;
    try {
      const buf = await Bun.file(`/proc/${pid}/environ`).arrayBuffer();
      const vars = new TextDecoder().decode(buf).split("\0");
      const entry = vars.find(v => v.startsWith("SteamAppId="));
      if (!entry) continue;

      const appId = entry.slice("SteamAppId=".length);
      if (appId && appId !== "0") {
        return appIdMap.get(appId) ?? shortcuts.get(appId) ?? `Unknown game (appid ${appId})`;
      }
    } catch {
      // process exited or not readable
    }
  }

  return null;
}

const libraryPaths = await getLibraryPaths();
const [appIdMap, shortcuts] = await Promise.all([buildAppIdMap(libraryPaths), loadShortcuts()]);
console.log(
  `Loaded ${appIdMap.size} installed games + ${shortcuts.size} shortcuts from ${libraryPaths.length} library path(s)`,
);

let lastGame: string | null = undefined!;

async function poll() {
  try {
    const game = await getRunningGame(appIdMap, shortcuts);
    if (game !== lastGame) {
      if (game) {
        console.log(`[${new Date().toISOString()}] Now playing: ${game}`);
      } else {
        console.log(`[${new Date().toISOString()}] Not playing anything`);
      }
      lastGame = game;
    }
  } catch (err) {
    console.error(`[${new Date().toISOString()}] Poll failed:`, err);
  }
}

await poll();
setInterval(poll, POLL_INTERVAL_MS);
